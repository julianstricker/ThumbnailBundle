...
class ThumbnailController extends Controller
{
    public function thumbnailAction(Request $request) {
        ...
        $placeholder = $request->get('placeholder', 'noimage.jpg');    
        $uploadsdir=$this->container->getParameter('uploadsdir');
        $imgname= $uploadsdir.$img;
        if (!is_file($imgname) || !is_readable($imgname)){
            $imgname=__DIR__.'/../../../../web/images/placeholder/'.$placeholder;
        ...
        if ($cachefile = $this->get('cache')->fetch('JSThumbnailBundle' . $cachename)) {
...
//                print_r($info);
            ...
            $this->get('cache')->save('JSThumbnailBundle' . $cachename, serialize($ImageData));
